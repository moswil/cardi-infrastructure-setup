# Create a VPC
resource "digitalocean_vpc" "keycloak_srv_vpc" {
  name     = var.vpc_name
...
resource "digitalocean_tag" "keycloak_srv_tags" {
  for_each = toset(var.mail_server_tags)
  name     = each.value
}

# Create the mail server node
resource "digitalocean_droplet" "keycloak_srv_droplet" {
  name       = var.hostname
  ...
  monitoring = true
  ipv6       = true
  vpc_uuid   = digitalocean_vpc.keycloak_srv_vpc.id
  ...
  tags = [for name in values(digitalocean_tag.keycloak_srv_tags)[*].id : name]
...
# Virtual IP
resource "digitalocean_reserved_ip" "keycloak_srv_reserved_ip" {
  region     = var.mail_server_region
  droplet_id = digitalocean_droplet.keycloak_srv_droplet.id
}
